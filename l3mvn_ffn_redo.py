...
from utils.SimulationContext import (
    SimulationContext,
)
...
from utils.util_helper import (
    ...
    random_action_goals,
    load_object_norm_inv_perplexity,
)

# Constants
ENV_THREADS = "OMP_NUM_THREADS"
MAPPINGS_FILE = "data/matterport_category_mappings.tsv"
# Setup environment
os.environ[ENV_THREADS] = "1"
...
def main_simulation_loop(
    ...
    object_norm_inv_perplexity = load_object_norm_inv_perplexity(sim_context.device)
            ...
            if len(frontier_score_list[e]) > 0:
                ...
                if max(frontier_score_list[e]) > 0.1:
                    if args.task_config == "tasks/objectnav_gibson.yaml":
                        g_reward = sim_context.infos[e]["g_reward"]
                        sim_context.g_process_rewards += g_reward
