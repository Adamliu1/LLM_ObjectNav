...
def main():
    ...
    _, planner_pose_inputs = initialize_planner_inputs_and_scores(args, num_scenes)
...
def main_simulation_loop(
    ...
    previous_frontier_movement = [None for _ in range(num_scenes)]
    # main training loop
    for step in range(args.num_training_frames // args.num_processes + 1):
        ...
        if l_step == args.num_local_steps - 1:
            ...
            for e in range(num_scenes):
                ...
                if len(target_point_map) > 0:
                    ...
                    object_clusters = sim_context.map_manager.extract_object_infos(e)
                    # this is approach directly select goal location (x,y)
                    goal_point = (
                        sim_context.navigation_manager.calculate_frontier_scores_v4(
                            e,
                    ...
                    goal_point_array = np.random.randint(
                        0, target_edge_map.shape[0], size=2
                    )
                    goal_point = tuple(goal_point_array)
                goal_points[e] = goal_point
        ...
        for e in range(num_scenes):
            ...
            # ------------------------------------------------------------------
            local_goal_maps[e][goal_points[e][0], goal_points[e][1]] = 1
