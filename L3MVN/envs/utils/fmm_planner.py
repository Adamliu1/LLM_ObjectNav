...
class FMMPlanner:
    def __init__(self, traversible, scale=1, step_size=5):
        ...
        self.traversible_around = cv2.dilate(
            (1 - self.traversible * 1).astype("uint8"), kernel
        )
    ...
    def set_multi_goal(self, goal_map):
        traversible_ma = ma.masked_values(
            self.traversible * 1, 0
        )  # mask掉障碍物层(0),只剩空白区域
        traversible_ma[goal_map == 1] = 0  # 除去障碍物层的地图，目标为0，其他为1
        dd = skfmm.distance(traversible_ma, dx=1)
        dd = ma.filled(
            dd, np.max(dd) + 1
        )  # 将mask的值替换为max(dd)+1, 即障碍物为最大距离
        self.fmm_dist = dd
