...
class ObjectGoal_LLM_Env(habitat.RLEnv):
    ...
    def __init__(self, args, rank, config_env, dataset):
        ...
        # setup original_action_space required by VectorEnv
        self.original_action_space = copy.deepcopy(self.action_space)
    ...
    def step(self, action):
        ...
        rgb = obs["rgb"].astype(np.uint8)
        depth = obs["depth"]
        # ORITIONAL SEM_EXP DOESN'T CONTAIN SEMANTIC
        semantic = self._preprocess_semantic(obs["semantic"])
        state = np.concatenate((rgb, depth, semantic), axis=2).transpose(2, 0, 1)
    ...
    def _preprocess_semantic(self, semantic):
        ...
        for i in range(len(se)):
            if se[i] >= len(self.scene.objects):
                print(
                    f"Sem not aligned with scene objects, skipping. sem id: {se[i]}  Scene objects: {len(self.scene.objects)}",
                )
                continue
