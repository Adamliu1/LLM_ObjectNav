...
from envs.habitat.objectgoal_env import ObjectGoal_Env21
...
class L3MVNAgent(ObjectGoal_Env21):
    ...
    def _plan(self, planner_inputs):
        ...
        if (stop and planner_inputs["found_goal"] == 1) or self.replan_count > 26:
            ...
            # NOTE: I MODIFIED HERE AS ORIGIONAL L3MVN HAS AN OUT OF BOUND ERROR
            # add the evolution angle Calculate potential new start points
            # based on the angle and start position
            eve_start_x = int(5 * math.sin(angle_st_goal) + start[0])
            eve_start_y = int(5 * math.cos(angle_st_goal) + start[1])

            # Ensure the indices do not exceed the map boundaries and are
            # not negative
