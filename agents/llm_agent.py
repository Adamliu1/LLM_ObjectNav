...
from envs.habitat.objectgoal_env_llm import ObjectGoal_LLM_Env
...
class LLM_Agent_Env(ObjectGoal_LLM_Env):
    ...
    def __init__(self, args, rank, config_env, dataset):
        ...
        if args.sem_gpu_id == -1:
            args.sem_gpu_id = config_env["habitat"]["simulator"]["habitat_sim_v0"][
                "gpu_device_id"
            ]

        self.device = args.device
        # ------------------------------------------------------------------
        ##### This is setted up for sem pridiction
        # ------------------------------------------------------------------
        ...
        self.last_loc = None  # last location
        self.collision_map = None
        self.visited = None
        self.visited_vis = None  # visited visulation
        self.last_action = None  # last action
        self.col_width = None  # collision width
        self.episode_n = 0
        self.replan_flag = 0
        # ------------------------------------------------------------------

        self.curr_loc = None  # current location
        self.count_forward_actions = None
        self.replan_count = 0  # replan count
        self.collision_n = 0  # collision count
        ...
        self.obs = None  # obs
        self.obs_shape = None
        self.eve_angle = 0  # elevation angle
    ...
    def plan_act_and_preprocess(self, planner_inputs):
        ...
        if planner_inputs["new_goal"]:
            self.info["replan_flag"] = False

        action = self._plan(planner_inputs)  # so here is actually how to get the action

        print("debug: collision_n: ", self.collision_n)
        if self.replan_count > self.args.num_local_steps - 5 or self.collision_n > 20:
            self.info["replan_flag"] = True
            self.collision_n = 0
        ...
        if action >= 0:
            ...
            if done and not self.info["success"]:
                if self.info["time"] >= self.args.max_episode_length - 1:
                    ...
                    self.replan_count > self.args.num_local_steps - 5
                ):  # THIS IS MODIFIED BASED ON CO-NAVGPT
            ...
            self.last_action = None
            self.info["sensor_pose"] = np.zero(3)
    ...
    def _plan(self, planner_inputs):
        ...
        exp_pred = np.rint(planner_inputs["exp_pred"])  # Exploread Area
        ...
        if stop and planner_inputs["found_goal"] == 1:
            ...
            ## add the evolution angle
            # Calculate potential new start points based on the angle and start position
            eve_start_x = int(5 * math.sin(angle_st_goal) + start[0])
            eve_start_y = int(5 * math.cos(angle_st_goal) + start[1])

            # Ensure the indices do not exceed the map boundaries and are not negative
            eve_start_x = max(0, min(eve_start_x, map_pred.shape[0] - 1))
            eve_start_y = max(0, min(eve_start_y, map_pred.shape[1] - 1))

            # Determine action based on the exploration prediction and current angles
            if exp_pred[eve_start_x, eve_start_y] == 0 and self.eve_angle > -60:
                action = 5  # Specific action (e.g., adjust angle)
                self.eve_angle -= 30
            elif exp_pred[eve_start_x, eve_start_y] == 1 and self.eve_angle < 0:
                action = 4  # Another specific action (e.g., adjust angle)
                self.eve_angle += 30
    ...
    def _preprocess_obs(self, obs, use_seg=True):
        ...
        if args.use_gtsem:
            ...
            for i in range(16):  # NOTE: this is because 16 categories
    ...
    def _visualize(self, inputs):
        ...
        if args.visualize:
            ...
            cv2.waitKey(1)
            # cv2.waitKey(0)  # NOTE: here for debug
